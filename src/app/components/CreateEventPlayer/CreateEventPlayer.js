"use client";
import "./CreateEventPlayer.css";
import { useState } from "react";

export default function CreateEventPlayer({ event, setEvent }) {
    const [keyword, setKeyword] = useState("");
    const [playerIds, setPlayerIds] = useState([]);
    async function onSubmit() {
        const response = await fetch(`/api/create-event-player`, {
            method: "POST",
            headers: {
        ...
        const eventPlayers = await response.json();
        setEvent({
            ...event,
            eventPlayers: [
                ...event.eventPlayers,
