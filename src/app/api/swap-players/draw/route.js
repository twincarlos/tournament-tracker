...
export async function PUT(req) {
    ...
    if (eventPlayer1.position === 1) {
        await sql `UPDATE Matches SET "eventPlayer1Id" = ${eventPlayer2.eventPlayerId} WHERE "matchId" = ${eventPlayer1.matchId};`;
        if (eventPlayer2.position === 1) {
            await sql `UPDATE Matches SET "eventPlayer1Id" = ${eventPlayer1.eventPlayerId} WHERE "matchId" = ${eventPlayer2.matchId};`;
        } else {
            await sql `UPDATE Matches SET "eventPlayer2Id" = ${eventPlayer1.eventPlayerId} WHERE "matchId" = ${eventPlayer2.matchId};`;
        };
    } else {
        await sql `UPDATE Matches SET "eventPlayer2Id" = ${eventPlayer2.eventPlayerId} WHERE "matchId" = ${eventPlayer1.matchId};`;
        if (eventPlayer2.position === 1) {
            await sql `UPDATE Matches SET "eventPlayer1Id" = ${eventPlayer1.eventPlayerId} WHERE "matchId" = ${eventPlayer2.matchId};`;
        } else {
            await sql `UPDATE Matches SET "eventPlayer2Id" = ${eventPlayer1.eventPlayerId} WHERE "matchId" = ${eventPlayer2.matchId};`;
    ...
    WHERE m."eventId" = ${eventId} AND m."matchStage" = 'Draw'
    ...
    return new Response(JSON.stringify(orderedDraw));
};